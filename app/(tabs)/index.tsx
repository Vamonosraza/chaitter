...
export default function HomeScreen() {
  ...
  const handleCreate = () => {
    console.log('Navigating to create companion screen');
    router.push('/(modal)/create-companion');
  };
  ...
  return (
    <View style={styles.container}>
      ...
      {(() => {
        // Extracted rendering logic for companions section
        if (loading) {
          return <ActivityIndicator size="large" color="#4285F4" style={{ marginTop: 24 }} />;
        }
        if (companions.length === 0) {
          return <Text style={styles.emptyText}>No AI companions yet. Tap "Create" to get started!</Text>;
        }
        return (
          <FlatList
            data={companions}
